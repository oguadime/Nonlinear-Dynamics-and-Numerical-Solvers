...
function NonlinearFEU1(u0, tau)
    ...
    % Function for the energy F(U), the integral of f
    F = @(u) -(u.^2) / 2 + (u.^4) / 4 + 1/4;
    ...
    % Main body of code
    for i = 2:Nsteps
        unew(i) = unew(i-1) + dt*(unew(i-1) - (unew(i-1)).^3);
    ...
    hold on;
    plot(t, unew, '*-', 'DisplayName', 'u_{FE}');
    xlabel('t');
    ylabel('u(t)');
    legend('show');
    title('Comparison of Forward Euler and ode45 Solutions');
    grid on;
