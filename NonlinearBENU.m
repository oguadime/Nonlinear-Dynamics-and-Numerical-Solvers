function NonlinearBENU(u0, tau)
    ...
    uBE = unew;        % Array to store solution

    % Main loop with variable time stepping
    while t < T
        % Define the implicit function for Backward Euler
        f = @(unext) unext - unew - tau * (unext - unext^3);
        
        % Solve for the next time step using fzero
        u_next = fzero(f, unew);  % Use the previous value as the initial guess
        
        % Estimate error by comparing with two half-steps
        f_half = @(u_half) u_half - unew - 0.5 * tau * (u_half - u_half^3);
        u_half = fzero(f_half, unew);
        f_half_next = @(u_half_next) u_half_next - u_half - 0.5 * tau * (u_half_next - u_half_next^3);
        ...
        if error_estimate > tol
            ...
            uBE(end+1) = unew; % Store the solution
    ...
    hold on;
    plot(tsteps, uBE, '*-', 'DisplayName', 'u_{BE}');
    xlabel('t');
    ylabel('u(t)');
    legend('show');
    title('Comparison of Backward Euler (Variable Step) and ode45 Solutions');
    grid on;
end
